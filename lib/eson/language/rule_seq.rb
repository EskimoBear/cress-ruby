...
't capture tokens."
      ...
      def missing_items_error_message(rule_names)
        names = rule_names.map{|i| ":".concat(i.to_s)}
                "One or more of the following Eson::Language::Rule.name's are not prese\
nt in the sequence: #{names.join(", ")}."
      ...
      def missing_rule_error_message(rule_name)
                "The Eson::Language::Rule.name ':#{rule_name}' is not present in the se\
quence."
      end
      ...
      #@param rule [Eson::Language::Rule] Given rule
