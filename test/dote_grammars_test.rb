...
describe Dote::DoteGrammars do
  ...
  describe "validate_tokenizer_cfg" do
    ...
    it "has no partial rules" do
      @lang.none?{|x| x.partial_status}.must_equal true
    end
  end
  ...
  describe "validate_format" do
    ...
    it "rules have s_attr line_no" do
      @lang.all?{|i| i.s_attr.include? :line_no}
        .must_equal true
    end
    it "rules have s_attr indent" do
      @lang.all?{|i| i.s_attr.include? :indent}
        .must_equal true
    end
    it "only colon has s_attr spaces_after" do
      @lang.get_rule(:colon).s_attr.include?(:spaces_after)
        .must_equal true
      (@lang.terms - [:colon])
        .none?{|i| @lang.get_rule(i).s_attr.include? :spaces_after}
        .must_equal true
    end
  end
...
describe Dote::Rule do
  ...
  describe "#to_s" do
    it "is a terminal rule" do
      @lang.get_rule(:element_divider).to_s.must_match /element_divider/
    end
    it "is a concatenation rule" do
      @lang.get_rule(:program).to_s.must_match /( := )/
    end
    it "is a alternation rule" do
      @lang.get_rule(:proc_identifier).to_s.must_match /( := )/
    end
    it "is a repetition rule" do
      @lang.get_rule(:sub_string_list).to_s.must_match /( := )/
    end
  ...
  it "has non-terminals" do
    @lang.get_rule(:value).partial_status.must_equal false
    first_set = @lang.get_rule(:value).first_set
    first_set.must_be_instance_of Array
  ...
  describe "starts with nonterminal" do
    it "is option rule" do
      @lang.get_rule(:element_set).partial_status.must_equal false
      first_set = @lang.get_rule(:element_set).first_set
      first_set.must_be_instance_of Array
